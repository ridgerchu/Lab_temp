...
class SpikingVGG(nn.Module):
    ...
    @staticmethod
    def make_layers(cfg, batch_norm=False, norm_layer=None, neuron: callable = None, att=None, **kwargs):
        ...
        for v in cfg:
            if v == 'M':
                layers += [layer.MaxPool2d(kernel_size=2)]
            else:
...
cfgs = {
    ...
    'F' : [64, 64, 'M', 128, 128, 128],
    'G' : [64, 128, 256, 256, 'M', 512, 512, 512, 512],
}
...
def spiking_vgg11_bn(pretrained=False, progress=True, norm_layer: callable = None, spiking_neuron: callable = None, **kwargs):
    ...
    return _spiking_vgg('vgg11_bn', 'G', True, pretrained, progress, norm_layer, spiking_neuron, **kwargs)


def spiking_vgg7(pretrained=False, progress=True, norm_layer: callable = None, spiking_neuron: callable = None, **kwargs):
    """
        :param pretrained: If True, the SNN will load parameters from the ANN pre-trained on ImageNet
        :type pretrained: bool
        :param progress: If True, displays a progress bar of the download to stderr
    ...
    return _spiking_vgg('vgg7', 'F', True, pretrained, progress, norm_layer, spiking_neuron, **kwargs)
